...
class BlogController extends AbstractController
{
    ...
    */
    public function new(Request $request, FlashyNotifier $flashyNotifier )
    {
        ...
        if($form->isSubmitted() && $form->isValid()){
            ...
            $article->setImage("https://picsum.photos/200/300?random=2");
    ...
    */
    public function delete(Request $request, Article $article, FlashyNotifier $flashyNotifier): Response
    {
        if ($this->isCsrfTokenValid('delete' . $article->getId(), $request->request->get('_token'))) {
            $entityManager = $this->getDoctrine()->getManager();
            $entityManager->remove($article);
            $entityManager->flush();
            $flashyNotifier->success('Successfully Deleeeted !');
